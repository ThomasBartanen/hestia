...
pub fn write_with_printpdf(s: Statement, p: Property) {
    ...
    current_layer.begin_text_section();
        ...
        current_layer.use_text(format!("Balance Forward: {:.2}", 0), BODY_SIZE, left_column, y_level, &font);
        current_layer.use_text(format!("Payment Received {:.2}", 0), BODY_SIZE, right_column, y_level, &font);
        y_level -= Mm(10.0);
        current_layer.use_text(format!("Outstanding Balance: {:.2}", 0), BODY_SIZE, right_column, y_level, &font);
        y_level -= Mm(10.0);
    ...
    y_level = Mm(15.0);
    current_layer.use_text(format!("Thank You"), BODY_SIZE, right_column, y_level, &font);
    y_level += Mm(10.0);
    current_layer.use_text(format!("Please Remit To: MAILING ADDRESS"), BODY_SIZE, left_column, y_level, &font);   
    current_layer.use_text(format!("Payment Due 1st of Coming Month"), BODY_SIZE, right_column, y_level, &font);
